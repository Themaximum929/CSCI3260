...
BezierCurve::BezierCurve(int numControlPoints)
{
    ...
    t = 0.5;
    controlPoints = std::vector<glm::vec2>(numControlPoints);
}
...
void BezierCurve::loadControlPoints(FILE* file)
{
  ...
  {
    fscanf(file, "%f %f", &controlPoints[i].x, &controlPoints[i].y);
  }
}
...
void BezierCurve::render()
{
  ...
  {
    glm::vec2 pt1 = controlPoints[i];
    glm::vec2 pt2 = controlPoints[i+1];
  ...
  if (eval_level) {
    ...
    std::vector<std::vector<glm::vec2> > evaluatedLevels;
    ...
    {
      std::vector<glm::vec2>& evaluatedPoints = evaluatedLevels[level];
      for (int i = 0; i < evaluatedPoints.size() - 1; i++)
      {
        glm::vec2& pt1 = evaluatedPoints[i];
        glm::vec2& pt2 = evaluatedPoints[i+1];
        glVertex2f(pt1.x, pt1.y);
        glVertex2f(pt2.x, pt2.y);
      }
    }
...
void BezierCurve::drawCurve()
{
  ...
  for (float p = 0.0; p <= 1.0f; p += 0.005f)
  {
    ...
    std::vector<glm::vec2>& lastLevel = evaluatedLevels[evaluatedLevels.size() - 1];
...
void BezierCurve::cursor_event(float x, float y, unsigned char keys)
{
  ...
  if (keys == 4 || keys == 212)
  {
    if (selected >= 0)
    {
      float dx = x - xp;
      float dy = y - yp;
      controlPoints[selected] += glm::vec2(dx, dy);
    }
    else
    {
      glm::vec2 xy(x,y);
      ...
      for (int i = 0; i < controlPoints.size(); i++)
      {
        if (glm::length(controlPoints[i] - xy) < thresh)
        {
          selected = i;
        }
      }
